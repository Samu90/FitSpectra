...
void Analysis(string FName, string Source,string output){
  ...
  TF1* spectrum = new TF1("SpectrumFit","[0] * exp(-( x-[1] )*( x-[1] )/( 2* [2]* [2])) + [3] / (exp( (x*[4]-(2*[1]*[1]/([1]+2*[1])))) + 1)+ [5] * exp(-( x-[6] )*( x-[6] )/( 2* [7]* [7])) + [8] / (exp( (x*[9]-(2*[6]*[6]/([6]+2*[6])))) + 1) + [10]*exp(-x*[11]-[12])+crystalball([13],[14],[15],[16],[17])",27,88);
  ...
  spectrum->SetParLimits(13,55000,65000);
  spectrum->SetParLimits(14,65,70);
  spectrum->SetParLimits(15,4,5.5);
  spectrum->SetParLimits(16,0,0.5);
  spectrum->SetParLimits(17,-11,-8.5);
  ...
  TF1* CB  = new TF1("CB","crystalball",xmin,xmax);
  ...
  CB->FixParameter(0,spectrum->GetParameter(13));
  CB->FixParameter(1,spectrum->GetParameter(14));
  CB->FixParameter(2,spectrum->GetParameter(15));
  CB->FixParameter(3,spectrum->GetParameter(16));
  CB->FixParameter(4,spectrum->GetParameter(17));

  

  gauss1->SetLineStyle(8);
  ...
  decExp->SetLineStyle(12);
  decExp->SetLineColor(kPink);
  CB->SetLineStyle(13);
  CB->SetLineColor(kMagenta);
